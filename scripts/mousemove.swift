// 验证用：把光标从 (x0,y0) 分步移到 (x1,y1)，逻辑坐标；末尾加 drag 则按住左键移动（拖窗口）。
// 用量表的悬停靠 Rust 轮询光标位置，端到端验证时需要真的把光标移过去；分步而非跳跃，接近真实鼠标轨迹。
// 用法：swiftc -O scripts/mousemove.swift -o /tmp/mousemove && /tmp/mousemove x0 y0 x1 y1 [drag]
import Foundation
import CoreGraphics
...
guard a.count >= 5, let x0 = Double(a[1]), let y0 = Double(a[2]), let x1 = Double(a[3]), let y1 = Double(a[4]) else {
    print("usage: mousemove x0 y0 x1 y1 [drag]")
    exit(2)
}
let drag = a.count > 5 && a[5] == "drag"
