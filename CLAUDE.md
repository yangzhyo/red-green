...
**审查** — PR 开出后跑 /code-review 双轴审：Standards（合不合仓库规范）、Spec（对不对得上 issue 验收标准）。评审毕**必在 PR 留一条小结评论**：有发现逐条贴，无发现也写明双轴通过、并点出 Spec 对齐了哪几项验收项（如「Standards 无发现；Spec 对齐 #N 全部验收项」）。没有小结＝审查未完成、不予合并——它既是审查步的完成定义，也是用户终审的可见抓手。
