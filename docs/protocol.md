...
两层结构：**hooks 写的是原始状态**（上图实线），**app 在显示层叠加已阅**（已完成/异常中止 + 前台聚焦 → 显示为空闲，不写回文件）。叫声在进入待确认/异常中止/轮到你/已完成时各响一次：节奏属状态——急促三连音/下坠双音/上扬双音/轻短单音，响度依次递减；音色属物种（听声辨项目），文件名 `皮肤-状态.wav`，由 `scripts/gen-sounds.mjs` 生成、随 .app 的 bundle resources 打包。受前台静默约束：该会话终端标签页在前台则不响（tmux 解析到活动 pane），被静默即消失，不补发。
