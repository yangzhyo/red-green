...
```
   ...
   │    (招手,蓝灯闪,静音)   不适用已阅：问题没回答就一直成立
   │
   └── SessionEnd（任何状态）→ 删除状态文件，宠物离场
```

两层结构：**hooks 写的是原始状态**（上图实线），**app 在显示层叠加已阅**（已完成/异常中止 + 前台聚焦 → 显示为空闲，不写回文件）。声音只在进入待确认（Glass）与异常中止（Basso）时响，且受前台静默约束（该会话终端标签页在前台则不响，tmux 解析到活动 pane）。
